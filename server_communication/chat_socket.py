    ...
    try:
        while True:
            try:
                ...
                if "encrypt" in income.decode("ascii"):
                    pub_key, priv_key = encryption.public_encryption()
                    give_key = "public_key "+str(priv_key)
                    conn.sendall(give_key.enconde("ascii"))

                elif income not in [b"q", b"end", b"bye", b""]:
                    decrypted = encryption.decrypt_string(income.decode("ascii"), priv_key)
